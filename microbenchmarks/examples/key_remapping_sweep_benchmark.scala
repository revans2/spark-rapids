...
// Key Remapping Sweep Benchmark
//
// This benchmark sweeps through different distributions of rows between left and right
// tables to demonstrate the performance impact of key remapping with string keys.
//
// Test Coverage:
// - Join Types: Hash and Sort (both inner joins)
// - Key Remapping: Enabled and Disabled
...
// - Key Type: String (20 characters average length)
// - Build Side Swapping: ENABLED (let optimizer choose smaller table)
//
// Note: Total rows limited so worst case (46340 left * 46340 right with 1 key)
// produces ~2.1B output rows, just under Int.MaxValue
//
// Expected Behavior:
// - Key remapping should improve performance for string keys
// - Benefit should be more pronounced with lower cardinality
// - Sort joins may show different characteristics than hash joins
//
...
val baseDir = "/data/tmp/key_remapping_sweep"
val outputTsvPath = s"$baseDir/key_remapping_sweep.tsv"
 ...
 * Uses string keys instead of long keys.
 */
def generateTestTables(
  ...
  // Create left table configuration with STRING key
  val leftConfig = TableGenConfig(
    numRows = leftRows,
    keyColumns = Seq(
      KeyColumnSpec(
        name = "join_key",
        dataType = "string",  // STRING instead of long
        minSeed = leftMinSeed,
        maxSeed = leftMaxSeed,
  ...
  // Create right table configuration with STRING key
...
def runBenchmark(
    ...
    enableRemapping: Boolean,
    leftRows: Long,
    rightRows: Long): JBR.BenchmarkResults = {
  
  println(s"\n========================================")
  println(s"Running: $testName")
  println(s"Strategy: $strategyName, Remapping: $enableRemapping")
  ...
  val config = JBR.JoinBenchmarkConfig(
    ...
    buildSide = JBR.AutoPickSmallerIfAllowed,  // Enable build side swapping
    optimizations = JBR.JoinOptimizations(
      allowBuildSideSwap = true,              // ENABLED: Let optimizer choose
      remapComplexKeysToInts = enableRemapping,  // Toggle remapping
...
def runFullBenchmark(): Unit = {
  println(s"Starting Key Remapping Sweep Benchmark")
  ...
  try {
    ...
    val totalTests = leftRowDistribution.size * cardinalityScenarios.size * 
                     overlapScenarios.size * strategies.size * remappingConfigs.size
    ...
    for {
      ...
      (remapName, enableRemapping) <- remappingConfigs
    } {
      ...
      val testName = f"L${leftPct}%03d_R${rightPct}%03d_${cardinality.name}_${overlap.name}_${strategyName}_${remapName}"
      ...
      try {
        ...
        case e: OutOfMemoryError =>
          ...
          tsvWriter.println(s"$testName\tOOM_ERROR\t$leftRows\t$rightRows\t0\t1\t0\t0.0\t0.0\t0.0\t0.0\t0.0\t0.0\t" +
            s"Inner\t$strategyName\tAuto(Smaller)\tN/A\t${if (enableRemapping) "swap,remap" else "swap"}\t\t\t\t\t")
          tsvWriter.flush()
...
println("=" * 80)
println("Key Remapping Sweep Benchmark")
println("=" * 80)
println()
println("This benchmark demonstrates the performance impact of key remapping")
println("for string keys in hash and sort-merge joins.")
println()
...
println(s"  Remapping configs: ${remappingConfigs.size}")
println(s"  Total tests: ${leftRowDistribution.size * cardinalityScenarios.size * overlapScenarios.size * strategies.size * remappingConfigs.size}")
println()

// Run the benchmark
runFullBenchmark()

println("\nDone! You can now analyze the results with the Python script.")
println(s"TSV file: $outputTsvPath")
println()
println("Recommended analysis:")
println("1. Plot average time vs left row percentage for remapping on/off")
println("2. Compare Hash vs Sort for remapping benefit")
println("3. Examine detailed timings to see remapping overhead vs join time savings")
println("4. Look for cardinality scenarios where remapping provides most benefit")
